...
import { FaPlus, FaEdit, FaTrash, FaTimes, FaToggleOn, FaToggleOff, FaSortNumericDown } from 'react-icons/fa';
import LoadingSpinner from '../common/LoadingSpinner';

const ServiceCategoriesManager: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        const response = await serviceCategoryApi.update(editingCategory.id, formData);
        if (response.success) {
        ...
        const response = await serviceCategoryApi.create(formData);
        if (response.success) {
          await loadCategories();
          setShowModal(false);
  ...
  const handleDelete = async (id: string) => {
    const confirmed = await confirm({
      ...
      message: 'Are you sure you want to delete this service category? Services using it will lose their category assignment.',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      type: 'danger',
    });
    ...
    try {
      const response = await serviceCategoryApi.delete(id);
      if (response.success) {
        await loadCategories();
  ...
  const handleToggleActive = async (id: string) => {
    try {
      const response = await serviceCategoryApi.toggleActive(id);
      if (response.success) {
        await loadCategories();
      } else {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 sm:mb-6">
        <div>
          <h2 className="text-3xl sm:text-4xl font-thin text-white">Service Categories</h2>
          <p className="font-medium text-[10px] sm:text-xs uppercase tracking-[0.2em] text-neutral-500 mt-2">Manage service category groupings with bilingual names</p>
        </div>
        <button
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-sm font-bold text-white truncate">
                        {language === 'en' ? category.name_en : category.name_sv}
                      </h3>
                      <p className="text-[10px] text-neutral-400 mt-0.5 truncate">
                        EN: {category.name_en} | SV: {category.name_sv}
                      </p>
                    </div>
