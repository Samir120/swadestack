...
const NewsletterVerify: React.FC = () => {
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        await newsletterApi.verify(token);
        setStatus('success');
      } catch (error: any) {
        ...
        if (msg.toLowerCase().includes('already')) {
          ...
          setStatus('error');
          setErrorMessage(msg || (language === 'en' ? 'Verification failed' : 'Verifieringen misslyckades'));
        }
      }
    };
          ...
          <div className="relative z-10">
            {status === 'loading' && (
              <div className="text-center py-8">
                ...
                <h2 className="text-xl font-thin text-gray-800 dark:text-white mb-2">
                  {language === 'en' ? 'Verifying your subscription...' : 'Verifierar din prenumeration...'}
                </h2>
            ...
            {status === 'success' && (
              <div className="text-center py-8">
                ...
                <h2 className="text-xl font-thin text-gray-800 dark:text-white mb-2">
                  {language === 'en' ? 'Subscription confirmed!' : 'Prenumeration bekr\u00e4ftad!'}
                </h2>
                <p className="text-gray-500 dark:text-neutral-400 text-sm mb-6">
                  {language === 'en'
                    ? 'Your email has been verified. You will now receive our newsletter.'
                    : 'Din e-post har verifierats. Du kommer nu att f\u00e5 v\u00e5rt nyhetsbrev.'}
                </p>
                <Link
                  to="/"
